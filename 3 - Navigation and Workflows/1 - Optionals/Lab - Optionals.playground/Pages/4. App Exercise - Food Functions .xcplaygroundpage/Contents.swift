...
func getMealLog(for mealType: String) -> Meal? {
    return meals[mealType]
}

if let breakfastLog = getMealLog(for: "Breakfast") {
    print("Breakfast Log: \(breakfastLog)")
} else {
    print("Breakfast not logged yet.")
}

if let lunchLog = getMealLog(for: "Lunch") {
    print("Lunch Log: \(lunchLog)")
} else {
    print("Lunch not logged yet.")
}
...
func retrieveMealLog() -> [String: Any] {
...
let mealLog = retrieveMealLog()
print("Retrieved Meal Log: \(mealLog)")
