...
func calculateArea(x: Double, y: Double) -> Double? {
    guard x > 0, y > 0 else {
        print("Invalid input. Both x and y must be greater than zero.")
        return nil
    }
    let area = x * y
    return area
...
if let positiveArea = calculateArea(x: 5.0, y: 3.0) {
    print("Area with positive numbers: \(positiveArea)")
} else {
    print("Error calculating area.")
}
if let negativeArea = calculateArea(x: -2.0, y: 4.0) {
    print("This won't be printed because of the guard statement.")
} else {
    print("Error calculating area due to negative input.")
}
...
func add(_ a: Int?, _ b: Int?) -> Int? {
    guard let unwrappedA = a, let unwrappedB = b else {
        print("Invalid input. Both parameters must have a value.")
        return nil
    }
    return unwrappedA + unwrappedB
}

if let result1 = add(5, 3) {
    print("Sum with non-nil numbers: \(result1)")
} else {
    print("Error adding numbers.")
}

if let result2 = add(10, nil) {
    print("This won't be printed because of the guard statement.")
} else {
    print("Error adding numbers due to nil parameter.")
}
...
func createUser() -> User? {
    ...
    guard let firstName = firstNameTextField.text,
        ...
        print("Invalid input. All text fields must have a value.")
        return nil
    }
...
if let createdUser = createUser() {
    print("User created: \(createdUser)")
} else {
    print("Error creating user due to missing information.")
}
...
if let createdUser = createUser() {
    ...
    let firstName = createdUser.firstName
    let lastName = createdUser.lastName
    let age = createdUser.age
